...
**注意：凡是建议/评估/判断类问题，不管看起来多简单，都进S1。**
...
| 触发条件 | 工作流文件 | 说明 |
|---------|----------|------|
| S0判断不是简单任务 | task-evaluation.md | 五维打分→决策→一行说明 |
| 分析/建议/评估/判断类问题 | problem-analysis.md | 定义→分解→模型→分析→总结→落地 |
| 文档/图片处理 | document-processing.md | 文本/图片输入处理 |
| 音频处理 | audio-processing.md | 会议纪要/学习笔记/喂给我学习 |
| 视频处理 | video-processing.md | 视频四大场景 |
| 选择工具/能力 | tool-selection.md | 先水产市场，评估效能/成本/风险 |
| Get笔记API | skills/getnote/references/api-details.md | 正确字段名和接口 |
| 浏览器/网页 | memory/browser-experience.md | 先start再open，传targetId |
| OCR需求 | ocr-toolchain.md | 图片/PDF/音视频分层工具链 |
