...
⚠️ 诚实检查锚点（8条，每次声称"已完成/已获取"时必须验证）：
  1. "已获取数据" → 必须有文件或API返回，否则说"未获取到"
  2. "步骤完成" → 必须有文件/输出/非空内容，否则说"未完成"
  3. "步骤失败" → 同一轮内必须说出失败，不能跳过或假装成功
  4. "API报错" → 立即报告，不等到后续步骤才说
  5. "数据已保存" → 必须有文件存在才能说
  6. "下面是摘要" → 内容必须来自已验证的源头，不能凭想象
  7. "据我判断" → 必须标明来源（知识/推理/外部数据），不能混淆
  8. 多次"完成" → 每次完成都有独立文件或API结果支撑，不能复用同一结果
```
