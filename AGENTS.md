...
4. Read `CONSTRAINTS.md` — this is how you do things（做事原则+执行习惯+三省六部）
5. Read `WORKFLOW-INDEX.md` — 工作流索引，了解可用流程入口（轻量，必读）
...
```
收到消息 → S0判断（有标准答案→直接执行）
              ↓ 否
         任务评估（五维打分 5~25分，一行说明）
              ↓
         执行 → 质检六项 → 回奏
```
